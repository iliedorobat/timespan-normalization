...
```bash
  ...
  java -jar build/libs/temporal-normalization-2.2.0.jar --historicalOnly --sanitize --expression="1/2 sec. 3 a. chr - sec. 2 p. chr."
  # Include future dates
  java -jar build/libs/temporal-normalization-2.2.0.jar --expression="1/2 sec. 3 a. chr - sec. 2 p. chr."
```

### Analyze temporal expressions in LIDO datasets
```bash
  # Analyze only historical temporal expressions without pre-sanitizing them
  java -jar build/libs/temporal-normalization-2.2.0.jar --historicalOnly=true --sanitize=false --analysis
```

### Normalize multiple temporal expressions:
```bash
  # Parse only historical temporal expressions without pre-sanitizing them
  java -jar build/libs/temporal-normalization-2.2.0.jar --historicalOnly --analysis
  # Include future dates
  java -jar build/libs/temporal-normalization-2.2.0.jar --analysis
```
